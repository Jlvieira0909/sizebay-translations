...
export function search(index: SearchIndexRow[], query: string): SearchHit[] {
  ...
  for (const row of index) {
    ...
    if (row.pathHaystack.includes(phrase))
      score += row.pathHaystack === phrase ? 200 : 120;
    ...
    for (const value of row.values) {
      ...
      else if (value.haystack.includes(phrase))
        valueScore += value.haystack.startsWith(phrase) ? 220 : 180;

      if (!valueScore && tokens.length > 1) {
        const present = tokens.filter((token) =>
          value.tokens.has(token)
        ).length;
    ...
    if (!score && tokens.length > 0) {
      const combined = `${row.pathHaystack} ${row.values
        .map((value) => value.haystack)
        .join(" ")}`;
