...
function detectCode(fileName) {
  ...
  for (const part of stem
    ...
    if (BY_LOWER.has(part.toLowerCase()))
      return BY_LOWER.get(part.toLowerCase());
  }
...
async function collect(dir, depth = 0) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory() && depth < 1)
      found.push(...(await collect(path, depth + 1)));
...
if (!source) {
  console.error(
    "Usage: node scripts/prepare-locales.mjs <folder with the .json files>"
  );
  process.exit(1);
}
...
await writeFile(
  ...
  `${JSON.stringify(
    { generatedAt: new Date().toISOString(), locales },
    null,
    2
  )}\n`,
  "utf8"
...
console.log(
  `\npublic/locales — ${locales.length} language${
    locales.length === 1 ? "" : "s"
  } installed\n`
);
for (const locale of locales) {
  console.log(
    `  ${locale.code.padEnd(6)} ${String(locale.keys).padStart(5)} texts   ${
      locale.file
    }`
  );
}
...
if (keyCounts.size > 1) {
  console.log(
    `\n  note: the files do not all have the same number of texts (${[
      ...keyCounts,
    ]
      .sort((a, b) => a - b)
      .join(", ")}).`
  ...
  console.log(
    "  That is fine — missing keys show up in the editor with a dashed border."
  );
}
