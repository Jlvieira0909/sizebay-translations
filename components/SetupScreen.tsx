...
import { fetchStandardLocale, loadManifest, type LocaleManifest } from '@/lib/defaults';
...
export function SetupScreen() {
  const { state, dispatch, activeLocales, loadedCodes, awaitingCodes, entries, changeCount } = useTranslator();
  const [manifest, setManifest] = useState<LocaleManifest | null | undefined>(undefined);
  ...
  /** In standard mode, picking a language is enough — the file is fetched for you. */
  useEffect(() => {
  ...
  return (
    <div className="sb-setup">
      <header className="sb-hero">
        <TapeBand />
        <p className="sb-eyebrow sb-hero__eyebrow">Sizebay · fitting room &amp; size chart</p>
        ...
        <p className="sb-hero__lede">
          Every text your shoppers read lives behind a key like <code>footer.add_to_cart</code> — but you know it as
          the <em>Add to cart</em> button. Search by the words you can see in the store, edit every language side by
          side, and download files that are ready to ship.
        </p>
      </header>

      {state.source === 'choose' ? (
        <div className="sb-steps sb-steps--single">
          <SourceChoice manifest={manifest} onPick={(source) => dispatch({ type: 'set-source', source })} />
        </div>
      ) : (
        <>
            ...
            <section className="sb-card" aria-labelledby="step-languages">
              <div className="sb-card__head">
                ...
                <p className="sb-card__note">
                  Codes come straight from <code>languages.all_langs</code>. Several are not ISO — Brazil is{' '}
                  <code>br</code>, Danish is <code>dk</code>, and <code>uk</code> means British English.
                </p>
              </div>
            ...
            <section className="sb-card" aria-labelledby="step-files">
              <div className="sb-card__head">
                ...
                <h2 className="sb-card__title" id="step-files">
                  {standardMode ? 'Loading the standard texts' : 'Where are your files?'}
                </h2>
                <p className="sb-card__note">
                  Everything stays in this tab. Nothing is uploaded anywhere, and the files on your computer are never
                  touched.
                </p>
              </div>
              ...
              <button
                ...
                onClick={() => dispatch({ type: 'set-source', source: standardMode ? 'upload' : 'standard' })}
                disabled={!standardMode && !manifest}
                title={!standardMode && !manifest ? 'No standard texts in this project yet' : undefined}
              >
                {standardMode ? 'Use my own files instead' : 'Use the standard texts instead'}
              </button>
            </section>
          </div>
          ...
          <div className="sb-launch">
            <p className="sb-launch__status" aria-live="polite">
              {ready ? (
                <>
                  <strong>{activeLocales.length}</strong> {activeLocales.length === 1 ? 'language' : 'languages'} ready ·{' '}
                  <strong>{entries.length}</strong> texts
                  {changeCount > 0 ? (
                    <>
                      {' '}
                      · <strong>{changeCount}</strong> unsaved {changeCount === 1 ? 'change' : 'changes'} kept
                    </>
                  ) : null}
                </>
              ) : standardMode ? (
            ...
            >
              {changeCount > 0 ? 'Back to editor' : 'Open editor'} <ArrowRightIcon size={16} />
            </button>
          </div>

          {!standardMode && state.selected.length > 0 && loadedCodes.length === 0 ? (
            <p className="sb-hint sb-hint--center">
              Languages are selected but no file matched them yet. Drop the files above, or use the dropdown on each
              file to say which language it is.
            </p>
          ) : null}
...
/** Per-language state while the standard files come in. */
  ...
  if (state.selected.length === 0) {
    return (
      <p className="sb-empty">
        Nothing selected yet. Choose the languages above and their texts load straight away — no files needed.
      </p>
    );
  }
    ...
    <div className="sb-files">
      <ul className="sb-filelist">
        {state.selected.map((code) => {
          ...
          return (
            <li key={code} className="sb-filelist__row" data-waiting={waiting || undefined}>
              <span className="sb-filelist__name">{file?.fileName ?? `${code}.json`}</span>
              ...
              <span className="sb-filelist__origin">
                {file ? (
                  <>
                    <CheckIcon size={13} /> {file.origin === 'standard' ? 'standard' : 'your file'}
                  </>
                ) : null}
              </span>
              <button
      ...
      {loadedCodes.length > 0 ? (
        <p className="sb-hint">
          You can still drop a file of your own over any of these — switch below and it replaces that language.
        </p>
      ) : null}
    </div>
  );
